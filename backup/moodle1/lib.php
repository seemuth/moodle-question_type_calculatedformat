...
/**
 * Calculated question type with number formatting conversion handler
 */
class moodle1_qtype_calculatedformat_handler extends moodle1_qtype_handler {
    ...
    /**
     * Appends the calculatedformat specific information to the question
     */
    public function process_question(array $data, array $raw) {
        ...
        if (isset($data['calculatedformat'][0]['numerical_units'])) {
            $numericalunits = $data['calculatedformat'][0]['numerical_units'];
        } else {
        ...
        if (isset($data['calculatedformat'][0]['dataset_definitions']['dataset_definition'])) {
            $datasetdefinitions = $data['calculatedformat'][0]['dataset_definitions']['dataset_definition'];
        } else {
            $datasetdefinitions = array();
        }
        $this->write_dataset_definitions($datasetdefinitions);

        // Write calculatedformat_records.
        $this->xmlwriter->begin_tag('calculatedformat_records');
        foreach ($data['calculatedformat'] as $calculatedrecord) {
            $record = array(
            ...
            $this->write_xml('calculatedformat_record', $record, array('/calculated_record/id'));
        }
        $this->xmlwriter->end_tag('calculatedformat_records');

        // Write calculatedformat_options.
        $options = array(
            'calculateformat_option' => array(
                'id'                             => $this->converter->get_nextid(),
                'synchronize'                    => 0,
                'single'                         => 0,
                'shuffleanswers'                 => 0,
        ...
        $this->write_xml('calculatedformat_options', $options, array('/calculatedformat_options/calculateformat_option/id'));
    }
}
